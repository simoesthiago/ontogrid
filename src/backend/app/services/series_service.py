from __future__ import annotations

from datetime import datetime

from sqlalchemy import func, or_, select
from sqlalchemy.orm import Session

from app.db.models import Dataset, DatasetVersion, Entity, MetricSeries, Observation, SeriesRegistry
from app.services.catalog_service import to_iso8601


def _parse_timestamp(value: str | None) -> datetime | None:
    if not value:
        return None
    return datetime.fromisoformat(value.replace("Z", "+00:00"))


class SeriesService:
    def list_series(
        self,
        session: Session,
        *,
        dataset_id: str | None,
        entity_id: str | None,
        metric_code: str | None,
        q: str | None,
        limit: int,
        offset: int,
    ) -> tuple[list[dict[str, object]], int]:
        query = select(MetricSeries, SeriesRegistry).outerjoin(SeriesRegistry, SeriesRegistry.metric_series_id == MetricSeries.id).order_by(MetricSeries.metric_name)
        if dataset_id:
            query = query.where(MetricSeries.dataset_id == dataset_id)
        if metric_code:
            query = query.where(MetricSeries.metric_code == metric_code)
        if entity_id:
            query = query.join(MetricSeries.observations).where(Observation.entity_id == entity_id)
        if q:
            term = f"%{q.lower()}%"
            query = query.where(
                or_(
                    func.lower(MetricSeries.metric_name).like(term),
                    func.lower(MetricSeries.metric_code).like(term),
                )
            )
        query = query.distinct()
        total = session.scalar(select(func.count()).select_from(query.order_by(None).subquery())) or 0
        rows = session.execute(query.offset(offset).limit(limit)).all()
        return (
            [
                {
                    "id": metric_series.id,
                    "dataset_id": metric_series.dataset_id,
                    "metric_code": metric_series.metric_code,
                    "metric_name": metric_series.metric_name,
                    "unit": metric_series.unit,
                    "temporal_granularity": metric_series.temporal_granularity,
                    "entity_type": metric_series.entity_type,
                    "latest_observation_at": to_iso8601(metric_series.latest_observation_at) or "",
                    "semantic_value_type": registry.semantic_value_type if registry else "observed",
                    "reference_time_kind": registry.reference_time_kind if registry else "instant",
                }
                for metric_series, registry in rows
            ],
            total,
        )

    def get_observations(
        self,
        session: Session,
        *,
        series_id: str,
        start: str | None,
        end: str | None,
        dataset_version_id: str | None,
        entity_id: str | None,
        limit: int,
        offset: int,
    ) -> dict[str, object] | None:
        series = session.get(MetricSeries, series_id)
        if series is None:
            return None

        if dataset_version_id:
            selected_version_id = session.scalar(
                select(DatasetVersion.id).where(
                    DatasetVersion.id == dataset_version_id,
                    DatasetVersion.dataset_id == series.dataset_id,
                )
            )
        else:
            selected_version_id = session.scalar(
                select(Dataset.latest_version_id).where(Dataset.id == series.dataset_id)
            )
        query = (
            select(Observation, Entity.name)
            .join(Entity, Entity.id == Observation.entity_id)
            .where(Observation.series_id == series_id)
            .order_by(Observation.time)
        )
        if selected_version_id:
            query = query.where(Observation.dataset_version_id == selected_version_id)
        if entity_id:
            query = query.where(Observation.entity_id == entity_id)
        if (start_dt := _parse_timestamp(start)) is not None:
            query = query.where(Observation.time >= start_dt)
        if (end_dt := _parse_timestamp(end)) is not None:
            query = query.where(Observation.time <= end_dt)

        rows = session.execute(query.offset(offset).limit(limit)).all()
        return {
            "series_id": series_id,
            "dataset_version_id": selected_version_id or "",
            "items": [
                {
                    "entity_id": observation.entity_id,
                    "entity_name": entity_name,
                    "timestamp": to_iso8601(observation.time) or "",
                    "value": observation.value_numeric if observation.value_numeric is not None else observation.value_text,
                    "unit": series.unit,
                    "quality_flag": observation.quality_flag,
                    "published_at": to_iso8601(observation.published_at) or "",
                }
                for observation, entity_name in rows
            ],
        }


series_service = SeriesService()
